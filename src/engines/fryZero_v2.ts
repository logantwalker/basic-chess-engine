...
// this is my attempt to reverse engineer wukong.js engine and rebuild it in typescript with my own "improvements" - if any
...
//the end goal is to hook this up to a react typescript front end, and also I want to play computer v computer games on chess.com/lichess.org
const wukong_ts = () => {
  // Global Constants //
  ...
  //   The number 0x88 is a hexadecimal integer (136, 210, 10001000). The rank and file positions are each represented by a nibble (hexadecimal digit), and the bit gaps simplify a number of computations to bitwise operations.
  ...
  let hashKey = 0
  let kingSquare = [e1, e8] // starting king squares
  ...
  // square attacked
  function isSquareAttacked(square: number, color: number) {
  ...
  // decode move
  function getMoveSource(move: number) {
    return move & 0x7f
  }
  function getMoveTarget(move: number) {
    return (move >> 7) & 0x7f
  }
  function getMovePromoted(move: number) {
    return (move >> 14) & 0xf
  }
  function getMoveCapture(move: number) {
    return (move >> 18) & 0x1
  }
  function getMovePawn(move: number) {
    return (move >> 19) & 0x1
  }
  function getMoveEnpassant(move: number) {
    return (move >> 20) & 0x1
  }
  function getMoveCastling(move: number) {
    return (move >> 21) & 0x1
  }
  /****************************\
     ============================
  ...
  // populate move list
  function addMove(moveList: object[], move: number) {
    let moveScore = 0
  ...
  // generate moves
  function generateMoves(moveList: any[]) {
          ...
          // castling
            ...
            if (castle & castlingSide[side][0]) {
              if (board[ks + 1] == e && board[ks + 2] == e) {
                if (
                  isSquareAttacked(ks, 1 - side) === false &&
                  isSquareAttacked(ks + 1, 1 - side) === false
                )
  ...
  // generate captures
  function generateCaptures(moveList: any[]) {
    for (let piece = P; piece <= k; piece++) {
      // @ts-ignore
      for (let pieceIndex = 0; pieceIndex < pieceList[piece]; pieceIndex++) {
        let sourceSquare = pieceList.pieces[piece * 10 + pieceIndex]
  ...
  function removePiece(piece: number, square: number) {
    ...
    //@ts-ignore: typescript hates indexing this array with a variable even though that variable is a number and is specified as such... sigh
    pieceList[piece]--
    //@ts-ignore
  ...
  // perft driver
  function perftDriver(depth: number) {
    if (depth == 0) {
      nodes++
      return
    }
  ...
  // perft test
  function perftTest(depth: number) {
    nodes = 0
  ...
  // set hash size
  function setHashSize(Mb: number) {
    hashTable = []
  ...
  // need to nail down these types later
  function readHashEntry(alpha: any, beta: any, bestMove: any, depth: any) {
    // init hash entry
  ...
  // set time control
  function setTimeControl(timeControl: any) {
    timing = timeControl
  }
  ...
  // move ordering
  function sortMoves(currentCount: number, moveList: any[]) {
    for (
  ...
  // sort PV move
  function sortPvMove(moveList: any[], bestMove: any) {
  ...
  // quiescence search
  function quiescence(alpha: any, beta: any) {
  ...
  // negamax search
  function negamax(alpha: any, beta: any, depth: any, nullMove: any) {
    pvLength[searchPly] = searchPly
    ...
    for (let count = 0; count < moveList.length; count++) {
      ...
      if (
        ...
        isSquareAttacked(kingSquare[side], side ^ 1) == false
      ) {
        takeBack()
        continue
      }
  ...
  function searchPosition(depth: any) {
    let start = Date.now()
    let score = 0
    let lastBestMove = 0
    ...
    for (let currentDepth = 1; currentDepth <= depth; currentDepth++) {
      ...
      let info = ""
      let uciScore: any
      if (typeof document != "undefined") uciScore = 0

      if (score >= -mateValue && score <= -mateScore) {
        info =
          "info score mate " +
          (-(score + mateValue) / 2 - 1).toString() +
          " depth " +
          currentDepth +
          " nodes " +
          nodes +
        ...
        if (typeof document != "undefined")
          // @ts-ignore
          uciScore = "M" + Math.abs(-(score + mateValue) / 2 - 1).toString()
      } else if (score >= mateScore && score <= mateValue) {
        info =
          "info score mate " +
          ((mateValue - score) / 2 + 1).toString() +
          " depth " +
          currentDepth +
          " nodes " +
          nodes +
        ...
        if (typeof document != "undefined")
          //ts-ignore
          uciScore = "M" + Math.abs((mateValue - score) / 2 + 1).toString()
      } else {
        info =
          "info score cp " +
          score +
  ...
  function setBoard(fen: string) {
    ...
    for (var rank = 0; rank < 8; rank++) {
      for (var file = 0; file < 16; file++) {
        ...
        if ((square & 0x88) == 0) {
          if (
            (fen[index].charCodeAt(0) >= "a".charCodeAt(0) &&
              fen[index].charCodeAt(0) <= "z".charCodeAt(0)) ||
            (fen[index].charCodeAt(0) >= "A".charCodeAt(0) &&
              fen[index].charCodeAt(0) <= "Z".charCodeAt(0))
          ) {
          ...
          if (
            fen[index].charCodeAt(0) >= "0".charCodeAt(0) &&
            fen[index].charCodeAt(0) <= "9".charCodeAt(0)
          ) {
            // @ts-ignore --- wtf is going on here
    ...
    if (fen[index] != "-") {
      var file = fen[index].charCodeAt(0) - "a".charCodeAt(0)
      var rank = 8 - (fen[index + 1].charCodeAt(0) - "0".charCodeAt(0))
  ...
  // print move
  function moveToString(move: any) {
  ...
  // print move list
  function printMoveList(moveList: any[]) {
  ...
  return {
    ...
    movePiece: function (userSource: any, userTarget: any, promotedPiece: any) {
      try {
        this.movePiece(userSource, userTarget, promotedPiece)
      } catch (e) {
        // @ts-ignore
        guiError(".movePiece()")
      }
    ...
    // board methods
    squareToString: function (square: any) {
      return coordinates[square]
    },
    promotedToString: function (piece: any) {
      // @ts-ignore
    ...
    setBoard: function (fen: string) {
      setBoard(fen)
    },
    generateFen: function () {
      return generateFen()
    },
    getPiece: function (square: any) {
      return board[square]
    },
    getSide: function () {
      return side
    ...
    // move manipulation
    moveFromString: function (moveString: any) {
      return moveFromString(moveString)
    },
    moveToString: function (move: any) {
      return moveToString(move)
    },
    makeMove: function (move: any) {
      makeMove(move)
    },
    moveStack: function () {
      return moveStack
    },
    loadMoves: function (moves: any) {
      loadMoves(moves)
    },
    getMoves: function () {
      return getMoves()
    ...
    getMoveSource: function (move: any) {
      return getMoveSource(move)
    },
    getMoveTarget: function (move: any) {
      return getMoveTarget(move)
    },
    getMovePromoted: function (move: any) {
      return getMovePromoted(move)
    },
    getMoveCapture: function (move: any) {
      return getMoveCapture(move)
    },
    getMoveCastling: function (move: any) {
      return getMoveCastling(move)
    },
    generateLegalMoves: function () {
      return generateLegalMoves()
    },
    printMoveList: function (moveList: any) {
    ...
    inCheck: function (color: any) {
      return isSquareAttacked(kingSquare[color], color ^ 1)
    },
    isSquareAttacked: function (square: any, color: any) {
      return isSquareAttacked(square, color)
    },

    // uci
    setHashSize: function (Mb: any) {
      setHashSize(Mb)
    },
